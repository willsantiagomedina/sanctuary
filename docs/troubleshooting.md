...
<h3>`bun install` or `bun run dev` fails</h3>
...
*   **Corrupt `node_modules`**: Try clearing Bun's cache and reinstalling:
    ```bash
    bun pm cache rm
    rm -rf node_modules
    bun install
    ```
*   **Workspace Configuration**: Verify the root `package.json` defines the `apps/*` and `packages/*` workspaces and that each package has correct `name` and dependency declarations so Bun can link everything consistently.

<h3>TypeScript errors (`tsc` or IDE errors)</h3>

*   **Dependencies**: Ensure all dependencies are installed (`bun install`).
...
*   **Build Errors**: If using a production build, ensure the build process (`bun run build`) completed without errors.
...
*   **Convex Reactivity**: Convex is designed for real-time updates. If data isn't refreshing, check:
    ...
    *   Is your local Convex development server (`bun --cwd packages/convex run dev`) running and correctly pushing changes?
...
*   **Dependencies**: Ensure `packages/convex/package.json` has all necessary dependencies declared and `bun install` has been run within the workspace.
